...
export default async function encounterMethodsJsonFetcher() {
  try {
    const { count } = await fetch('https://pokeapi.co/api/v2/encounter-method').then(response => response.json() as Promise<{ count: number }>);
    ...
    console.log('❌ -> Fetch failed for Encounter Methods.', error);
  }
}

encounterMethodsJsonFetcher();
